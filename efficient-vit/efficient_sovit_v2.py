...
class Attention_2(nn.Module):
    def __init__(self, dim, window_size=None, num_heads=3, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
...
class StokenAttention(nn.Module):
    def __init__(self, dim, stoken_size, n_iter=1, num_heads=3, qkv_bias=False, qk_scale=None, attn_drop=0.,
                 proj_drop=0.):
...
class EfficientViT(nn.Module):
    def __init__(self, config, channels=512, selected_efficient_net=0):
        ...
        self.sovit= StokenAttention(3, stoken_size=[8,8])
    ...
    def forward(self, img, mask=None):
        ...
        p = self.patch_size
        x2=self.sovit(img)
        x = self.efficient_net.extract_features(x2)
        # print(x.shape)
        # x2=self.sovit(x)

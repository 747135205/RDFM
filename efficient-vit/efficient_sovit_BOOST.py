...
def boost_multi_window(x,label):
    lossing = label - x
    if(label[0]==1):
        for index, lossin in enumerate(lossing):
            if(lossin>=0.5 and lossin<1):
                x[index]=0.7*x[index]
            elif(lossin>0 and lossin<0.5):
                x[index]=1.3*x[index]
    if(label[0]==0):
        for index, lossin in enumerate(lossing):
            if(lossin>-0.5 and lossin<0):
                x[index]=0.7*x[index]
            elif(lossin>-1 and lossin<=-0.5):
                x[index]=1.3*x[index]
    return x
...
class EfficientViT(nn.Module):
    def __init__(self, config, channels=512, selected_efficient_net=0):
        ...
        self.sovit= StokenAttention(1280, stoken_size=[8,8])
    ...
    def forward(self, img,mask=None):
        ...
        x=self.mlp_head(x)
        # label=label.cuda()
        # x=boost_multi_window(x,label)
        # _,_,_,preds=check_correct(x)

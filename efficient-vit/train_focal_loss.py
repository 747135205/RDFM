...
#0.4
#TRAINING_DIR = os.path.join(DATA_DIR_2, "training_set_04")
#VALIDATION_DIR = os.path.join(DATA_DIR_2, "validation_set_04")

#0.8
#TRAINING_DIR = os.path.join(DATA_DIR_2, "training_set_32")
#VALIDATION_DIR = os.path.join(DATA_DIR_2, "validation_set_32")
...
def read_frames(video_path, train_dataset, validation_dataset,opt,config):
    ...
    if label == 0:
        min_video_frames = max(int(config['training']['frames-per-video'] * config['training']['rebalancing_real']),4) # Compensate unbalancing
    else:
        min_video_frames = max(int(config['training']['frames-per-video'] * config['training']['rebalancing_fake']),4)
...
if __name__ == "__main__":
    ...
    # # scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=opt.num_epochs,eta_min=0.00001)
    # starting_epoch = 0
...
#pool存在是为了并行处理的实现，这段代码作用为并行地将视频文件用read_frames处理
    ...
    for t in range(starting_epoch, opt.num_epochs + 1):
        ...
        for index, (images, labels) in enumerate(dl):
            ...
            # sovie_pred,swin_pred,y_pred = model(images)
            # y_pred=0.1*(sovie_pred+swin_pred)+y_pred
            y_pred = model(images)
            y_pred = y_pred.cpu()

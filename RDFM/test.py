...
def read_frames(video_path, videos,opt,config):
    #video_path代表了之前进行的一系列帧提取的图片文件夹
    # Get the video label based on dataset selected
    method = get_method(video_path, DATA_DIR)  #video_path:'H:\\dataset_fake_detection\\dataset\\test_set\\DFDC\\acfaxepklq'
    #BASE_DIR="H:\\dataset_fake_detection"
    #DATA_DIR = os.path.join(BASE_DIR, "dataset") H:\\dataset_fake_detection\\dataset
    ...
    frames_number = len(os.listdir(video_path))  #因而这个列表的长度就代表文件的个数
    frames_interval = int(frames_number / opt.frames_per_video)
    frames_paths = os.listdir(video_path)  #获取video_path下的文件名列表
    frames_paths_dict = {}
    ...
    for key in frames_paths_dict.keys():
        for index, frame_image in enumerate(frames_paths_dict[key]):
            ...
            transform = create_base_transform(config['model']['image-size']) #修改过
    ...
    for index, video in enumerate(videos):
        ...
        for key in video:
            ...
            for i in range(0, len(video_faces), opt.batch_size):
                ...
                torch.no_grad()  # shutdown gradient calculating
                try:
                 pred = model(faces)
                 #pred, features = model(faces)
                 #print(features.size())
